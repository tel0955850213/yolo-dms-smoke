    ...
    try:
        ...
        while True:
            try:
                ...
                if 30.5 <= raw_temp <= 45.0:
                    if raw_temp > 35.5:
                        compensated_temp = 36.2 + (raw_temp % 0.5)
                    else:
                        compensated_temp = raw_temp + 1.3
...
def send_discord_alert(alert_type, frame):
    ...
    def _send():
        try:
            ...
            print(f"✅ [Discord] {alert_type} 警報發送成功")
        except Exception as e:
...
def yolo_inference_loop():
    ...
    print("🚀 超級 DMS 系統 v12 (Web 影像串流版) 啟動！")
...
def index():
    ...
    return """
    <html>
        <head><title>Super DMS System Live</title></head>
        <body style="background-color: black; color: white; text-align: center; padding-top: 50px;">
            <h2>Jetson Orin Nano - DMS 系統即時影像</h2>
            <img src="/video_feed" style="border: 2px solid cyan; border-radius: 10px; width: 640px; height: 480px;">
        </body>
    </html>
    """
...
if __name__ == "__main__":
    ...
    # 2. 在主執行緒啟動網頁伺服器，對外開放 Port 5000
    print("🌐 [Web Server] 即時影像伺服器已啟動！")
    print("👉 請在 Mac 的瀏覽器輸入： http://<你的Jetson IP>:5000")
