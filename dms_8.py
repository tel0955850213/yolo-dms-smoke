                ...
                if tf.complete:
                    ...
                    if msg.type == 0x0A15 and len(msg.data) >= 4:
                        ...
                        if HR_MIN <= raw_hr <= HR_MAX:
                            ...
                            with emergency_lock:
                                if emergency_mode and emergency_reason == "heartbeat":
                                    emergency_mode = False
                                    emergency_reason = ""
...
def vosk_listen_thread():
    ...
    stream.start_stream()
    print("🎤 [Vosk] 麥克風就緒，監聽：救命 / 幫助 / 幫幫忙 / 救我 / 取消")
    ...
    while True:
        try:
            ...
            if rec.AcceptWaveform(data):
                ...
                # 偵測到求救詞 → 觸發緊急警報
                if any(k in text for k in TRIGGER_WORDS):
                ...
                # 偵測到「取消」 → 解除緊急警報
                elif any(k in text for k in CANCEL_WORDS):
                    with emergency_lock:
                        if emergency_mode:
                            emergency_mode   = False
                            emergency_reason = ""
                            print("✅ [Vosk] 駕駛說「取消」，解除緊急警報")
                        else:
                            print("🎤 [Vosk] 偵測到「取消」，但目前無警報，略過")
        except Exception:
            pass
