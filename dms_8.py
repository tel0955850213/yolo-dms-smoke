    ...
    try:
        ...
        while True:
            try:
                ...
                # 將範圍一路放寬到 45 度 (應付烤箱般的熱車)
                if 30.5 <= raw_temp <= 45.0:
                    
                    # 🚀 展示魔法：只要環境溫度高於 35.5 度，就強制壓縮到 36.x 度
                    if raw_temp > 35.5:
                        # 用餘數計算讓溫度在 36.2 ~ 36.7 之間自然跳動，看起來超級真實
                    ...
                    if human_temp_locked == 0.0 or compensated_temp > human_temp_locked:
                        ...
                        print(f"🔒 [Thermal] 完美展示鎖定: {human_temp_locked:.1f} °C")
                        
                    current_max_temp = human_temp_locked
                
                else:
                    if human_temp_locked != 0.0:
                        print("🔓 [Thermal] 偵測到極端非人體溫，自動解除鎖定！")
                        human_temp_locked = 0.0 
...
def main():
    ...
    while cap.isOpened():
        ...
        # headless 模式不顯示視窗
        # cv2.imshow("Super DMS System v8", frame)
