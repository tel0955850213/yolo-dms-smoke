    ...
    try:
        ...
        while True:
            try:
                ...
                # 裁切出正中央的駕駛座區域 (高: 6~18, 寬: 10~22)，避開車頂跟車窗
                driver_roi = img_temp[6:18, 10:22]
                raw_temp = np.max(driver_roi)
                # --- 狙擊鏡魔法 結束 ---
                
                print(f"👉 [Thermal Log] 駕駛座 ROI 最高溫: {raw_temp:.2f} °C")
                
                # 放寬範圍，因車內可能很熱
                if 30.5 <= raw_temp <= 38.5:
                    
                    # 動態保護：如果車廂已經很熱(超過35.5度)，就取消補償，避免破表
                    if raw_temp > 35.5:
                        compensated_temp = raw_temp 
                    else:
                    ...
                    if human_temp_locked == 0.0 or compensated_temp > human_temp_locked:
                        ...
                        print(f"🔒 [Thermal] 鎖定駕駛溫度: {human_temp_locked:.1f} °C")
                        
                    current_max_temp = human_temp_locked
                
                else:
                    if human_temp_locked != 0.0:
                        print("🔓 [Thermal] 偵測到異常環境溫，自動解除鎖定！")
                        human_temp_locked = 0.0 
                        ...
                        if 40 < raw_hr < 150:
                            if len(hr_history) > 0:
                                ...
                                if abs(raw_hr - last_avg) > 15:
                                    print(f"🛡️ [Radar Log] 擋下動作突波: {raw_hr:.0f} bpm (維持平穩)")
                                    raw_hr = last_avg + (2.0 if raw_hr > last_avg else -2.0) 
...
def main():
    ...
    print("🚀 超級 DMS 系統 v7 (車廂抗熱 ROI 版) 啟動！")
    ...
    while cap.isOpened():
        ...
        # Headless 模式防閃退，維持註解狀態
        # cv2.imshow("Super DMS System v7", frame)
