...
# 硬體設定
CAMERA_INDEX = 0               # 若為 USB 鏡頭通常為 0 或 1
RADAR_PORT   = '/dev/ttyUSB0'  # 你的雷達序列埠
RADAR_BAUD   = 1382400
    ...
    try:
        ...
        while True:
            try:
                ...
                # --- 額溫槍補償演算法 (解決卡在34度的問題) ---
                if 31.0 <= raw_temp <= 35.5:
                    # 表面溫度補償值，你可以根據實際額溫槍的讀數微調這個 2.3
                    OFFSET = 2.3 
                    current_max_temp = raw_temp + OFFSET
                else:
                    # 如果超出人體範圍(例如熱水50度或冰塊10度)，直接顯示真實表面溫度
...
def main():
    ...
    while cap.isOpened():
        ...
        # 如果你是透過遠端 SSH 無畫面執行，請把下面兩行前面加上 '#' 註解掉
       # cv2.imshow("Super DMS System v4", frame)
